...
export default function TrainerLoginPage() {
  ...
  const checkTrainerRole = async (userId: string): Promise<boolean> => {
    try {
      ...
      return userData.role === 'TRAINER' || userData.role === 'ADMIN';
    } catch (err) {
  ...
  const handleSignIn = async (
    ...
    try {
      ...
      const isTrainer = await checkTrainerRole(user.uid);

      if (!isTrainer) {
        setError('Access denied. Trainer accounts only.');
        await signOutUser();
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0f1419] px-4">
      <div className="w-full max-w-md bg-[#1a1f29] rounded-lg shadow-xl p-8 border border-[#F39C12]/20">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-[#F39C12] mb-2">
            K9 Trainer 🎓
          </h1>
          <p className="text-gray-400">Trainer Dashboard</p>
        </div>
